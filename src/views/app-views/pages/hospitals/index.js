...
const HospitalsList = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/medical/getHospitals`,
        {
          headers: {
      ...
      setHospitalData(data); // Save the response data to state
    } catch (error) {
  ...
  const deleteHospital = async (elm) => {
    confirm({
      centered: true,
      ...
      async onOk() {
        try {
          const response = await axios.delete(
            `${process.env.REACT_APP_BASE_URL}/medical/deleteHospitalData?hospitalId=${elm._id}`,
            {
              headers: {
                Authorization: `Bearer ${token}`, // Use Bearer authentication with the access token
              },
  ...
  return (
    <div>
      <h2>{t("Hospital List")}</h2>
      <div style={{ textAlign: "right", margin: "10px 0" }}>
        <Button type="primary" onClick={() => setOpenModal(true)}>
          <PlusOutlined /> {t("Add Hospital")}
        </Button>
      </div>
