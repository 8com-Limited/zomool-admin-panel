...
const AddEditFeedModal = ({ openModal, setOpenModal, data, setData }) => {
  ...
  const handleCancel = () => {
    setOpenModal(false);
    setData({
      service: defaultService(servicesData, "Feed")[0],
      subService: defaultService(servicesData, "Feed")[1],
    });
  };
  ...
  const handleFeed = async () => {
    try {
      ...
      let request = await axios.post(
        `${process.env.REACT_APP_BASE_URL}/feed/addFeedData`,
        form,
        {
          headers: {
  ...
  const fetchData = async () => {
    try {
      ...
      setData((prev) => ({
        ...prev,
        service: defaultService(data, "Feed")[0],
        subService: defaultService(data, "Feed")[1],
      }));
    } catch (error) {
  ...
  const UpdateFeed = async () => {
    try {
      ...
      form.append("phone", phoneRef?.current?.state?.value);
      form.append("feedId", data._id);
      let request = await axios.put(
        `${process.env.REACT_APP_BASE_URL}/feed/updateFeedData`,
        form,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const props = {
    onChange(info) {
      setData((prev) => ({
        ...prev,
        feedImage: info.file.originFileObj,
      }));
    },
  };
  ...
  return (
    <div>
      <Modal
        title={data?._id ? t("Update Feed") : t("Add Feed")}
        visible={openModal}
        ...
        footer={[
          ...
          data?._id ? (
            <Button key="update" type="primary" onClick={UpdateFeed}>
              {t("Update")}
            </Button>
          ) : (
            <Button key="add" type="primary" onClick={handleFeed}>
              {t("Add")}
            </Button>
          ),
        ]}
      >
        <form action="">
          ...
          <div>
            <h5 style={{ marginBottom: "0px" }}>{t("Feed Name")}</h5>
            <Input
          ...
          <div>
            <h5 style={{ marginBottom: "0px" }}>{t("Feed Image")}</h5>
            {data?._id ? (
              <Image
                width={200}
                src={`${process.env.REACT_APP_BASE_URL}/${data.image}`}
              />
            ) : data?.feedImage instanceof File ||
              data?.feedImage instanceof Blob ? (
              <Image width={200} src={URL.createObjectURL(data.feedImage)} />
            ) : null}
