...
const AddEditHospitalModal = ({ openModal, setOpenModal, data, setData }) => {
  ...
  const handleCancel = () => {
    ...
    setData((prev) => ({
      type: "Pharmacies",
      service: defaultService(servicesData, "Medical", "Pharmacies")[0],
      subService: defaultService(servicesData, "Medical", "Pharmacies")[1],
    }));
  };
  ...
  const handleAddPharmacy = async () => {
    try {
      ...
      let request = await axios.post(
        `${process.env.REACT_APP_BASE_URL}/medical/addPharmaciesData`,
        form,
        {
          headers: {
  ...
  const fetchData = async () => {
    try {
      ...
      setData((prev) => ({
        ...prev,
        service: defaultService(data, "Medical", "Pharmacies")[0],
        subService: defaultService(data, "Medical", "Pharmacies")[1],
      }));
    } catch (error) {
  ...
  const UpdatePharmacy = async () => {
    try {
      ...
      form.append("phone", phoneRef?.current?.state?.value);
      form.append("pharmaId", data._id);
      let request = await axios.put(
        `${process.env.REACT_APP_BASE_URL}/medical/updatePharmaciesData`,
        form,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <div>
      <Modal
        title={data?._id ? t("Update Pharmacies") : t("Add Pharmacies")}
        visible={openModal}
        ...
        footer={[
          ...
          data?._id ? (
            <Button key="update" type="primary" onClick={UpdatePharmacy}>
              {t("Update")}
            </Button>
          ) : (
            <Button key="add" type="primary" onClick={handleAddPharmacy}>
              {t("Add")}
            </Button>
          ),
        ]}
      >
        <form action="">
          ...
          <div>
            <h5 style={{ marginBottom: "0px" }}>{t("Pharmacy Name")}</h5>
            <Input
