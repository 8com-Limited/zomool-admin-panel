...
import AddEditAccessoriesModal from "./Modals/AddEditAccessoriesModal";
...
const Accessories = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/accessories/getAccessories`,
        {
          headers: {
      ...
      setAccessoriesData(data); // Save the response data to state
    } catch (error) {
  ...
  const deleteAccessories = async (elm) => {
    confirm({
      centered: true,
      ...
      async onOk() {
        try {
          const response = await axios.delete(
            `${process.env.REACT_APP_BASE_URL}/accessories/deleteAccessories?accessorieId=${elm._id}`,
            {
              headers: {
                Authorization: `Bearer ${token}`, // Use Bearer authentication with the access token
              },
          ...
          fetchData();
          toast.success("Accessori deleted successfully !!!");
        } catch (error) {
          toast.error("Error while deleting Accessori");
          console.log(error.message, "error");
        }
      },
      onCancel() {
  ...
  return (
    <div>
      <h2>{t("Accessories List")}</h2>
      <div style={{ textAlign: "right", margin: "10px 0" }}>
        <Button type="primary" onClick={() => setOpenModal(true)}>
          <PlusOutlined /> {t("Add Accessories")}
        </Button>
      </div>
