import { Button, Image, Input, Modal, Select, Upload, Space } from "antd";
...
const AddEditAccessoriesModal = ({
  ...
  const handleCancel = () => {
    setOpenModal(false);
    setData((prev) => ({
      service: defaultService(servicesData, "Accessories")[0],
      subService: defaultService(servicesData, "Accessories")[1],
    }));
  };
  ...
  const handleAccessories = async () => {
    try {
      ...
      let request = await axios.post(
        `${process.env.REACT_APP_BASE_URL}/accessories/addAccessories`,
        form,
        {
          headers: {
  ...
  const fetchData = async () => {
    try {
      ...
      setData((prev) => ({
        ...prev,
        service: defaultService(data, "Accessories")[0],
        subService: defaultService(data, "Accessories")[1],
      }));
    } catch (error) {
  ...
  const UpdateAccessories = async () => {
    try {
      ...
      form.append("phone", phoneRef?.current?.state?.value);
      form.append("accessorieId", data._id);
      let request = await axios.put(
        `${process.env.REACT_APP_BASE_URL}/accessories/updateAccessories`,
        form,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const props = {
    onChange(info) {
      setData((prev) => ({
        ...prev,
        accessoriesImage: info.file.originFileObj,
      }));
    },
  };
  ...
  return (
    <div>
      <Modal
        title={data?._id ? t("Update Accessories") : t("Add Accessories")}
        visible={openModal}
        ...
        footer={[
          ...
          data?._id ? (
            <Button key="update" type="primary" onClick={UpdateAccessories}>
              {t("Update")}
            </Button>
          ) : (
            <Button key="add" type="primary" onClick={handleAccessories}>
              {t("Add")}
            </Button>
          ),
        ]}
      >
        <form action="">
          ...
          <div>
            <h5 style={{ marginBottom: "0px" }}>{t("Accessories Name")}</h5>
            <Input
          ...
          <div>
            <h5 style={{ marginBottom: "0px" }}>{t("Accessories Image")}</h5>
            {data?._id ? (
              <Image
                width={200}
                src={`${process.env.REACT_APP_BASE_URL}/${data.image}`}
              />
            ) : data?.accessoriesImage instanceof File ||
              data?.accessoriesImage instanceof Blob ? (
              <Image
                width={200}
                src={URL.createObjectURL(data.accessoriesImage)}
              />
            ) : null}
...
export default AddEditAccessoriesModal;
